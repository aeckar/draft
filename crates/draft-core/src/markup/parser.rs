...
macro_rules! rule {
    ($name:ident, $body:expr $(,)?) => {
        #[inline(always)]
        pub fn $name(tape: TokenStream<'a>) -> Option<AstOutput<'a>> {
...
impl<'a> Grammar {
    rule!(markup, |mut tape| {
        ...
        while let Some(AstOutput { completed, rest }) = Self::top_level_element(tape) {
            children.push(completed);
            tape = rest
        }
        AstOutput::some(node::branch(rule::Markup, children, meta::None), tape)
    });
    ...
    rule!(heading, |mut tape| {
        ...
        if let Some(child) = try_token!(tape, HeadingMarker) {
            ...
            if let Some(child) = try_token!(tape, Newline) {
                children.push(child);
                return AstOutput::some(node::branch(rule::Heading, children, meta::None), tape);
            }
        }
        None
    });
    ...
    rule!(line, |mut tape| {
        ...
        if let Some(b) = try_token!(tape, Newline) {
            return AstOutput::some(node::branch(rule::Line, vec![a, b], meta::None), tape);
        }
        None
    });
    ...
    rule!(line_element, |mut tape| {
        ...
        if let Some((choice, child)) = res {
            return AstOutput::some(node::branch(rule::LineElement, vec![child], choice), tape);
        }
        for (choice, handler) in rule_options![len; format, link, embed, macro_rule] {
            if let Some((child, jump)) = handler(tape) {
                return AstOutput::some(node::branch(rule::LineElement, vec![child], choice), jump);
            }
        }
        None
    });
    ...
    rule!(paragraph, |mut tape| {
        ...
        AstOutput::some(node::branch(rule::None, children, meta::None), tape)
    });
    ...
    rule!(format, |mut tape| {
        ...
        AstOutput::some(node::branch(rule::Format, vec![a, b, c], meta::None), tape)
    });

    //InferredLink | LinkMarker & linkTarget
    rule!(link, |mut tape| {
        match try_token!(tape, InferredLink) {
            Some(a) => AstOutput::some(node::branch(rule::Link, vec![a], meta::Choice(0)), tape),
            None => {
                let a = try_token!(tape, LinkMarker)?;
                let (b, tape) = Self::link_target(tape)?;
                let ab = node::branch(rule::None, vec![a, b], meta::None);
                AstOutput::some(node::branch(rule::Link, vec![ab], meta::Choice(1)), tape)
            }
        }
    });
    ...
    rule!(embed, |mut tape| {
        ...
        let (b, tape) = Self::link_target(tape)?;
        AstOutput::some(node::branch(rule::Embed, vec![a, b], meta::None), tape)
    });

    rule!(link_target, |mut tape| {
        let (_, res) = token_options![tape; LinkBody, LinkAliasBody];
        if let Some((choice, child)) = res {
            return AstOutput::some(node::branch(rule::LinkTarget, vec![child], choice), tape);
        }
        None
    });
    ...
    rule!(list, |mut tape| {
        ...
        let a = node::branch(rule::None, children_a, meta::None);
        AstOutput::some(node::branch(rule::List, vec![a], meta::None), tape)
    });
    ...
    pub fn list_item(mut tape: TokenStream<'a>, parent: &mut AstNode<'a>) -> Result<'a> {
        ...
        AstOutput::some(node::branch(rule::ListItem, vec![a, b], meta::None), tape)
    }

    rule!(line_quote, |mut tape| {
        let a = try_token!(tape, LineQuoteMarker)?;
        let (b, tape) = Self::link_target(tape)?;
        AstOutput::some(node::branch(rule::LineQuote, vec![a, b], meta::None), tape)
    });
    ...
    rule!(macro_rule, |mut tape| {
        ...
        AstOutput::some(node::branch(rule::Macro, vec![a, b, c], meta::None), tape)
    });
}
